...
|in：出现在array |{ket:{$in:value}}|db.<collection_name>.find({"奖学金":{$in:'国家奖学金'}} ||
|nin：未出现在array |{ket:{$nin:value} |db.<collection_name>.find({"奖学金":{$nin:'国家奖学金'}} | |
...
<B> 例10 将student_name为李想的文档的gender更新为女.</B>
```javascript
