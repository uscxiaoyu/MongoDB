...
用于说明时间类型的document, 如{ $type: "timestamp" }或{ $type: "date" }. 注意, timestamp和date必须为小写.
...
语法格式: `{$rename: { <field1>: <newName1>, <field2>: <newName2>, ... } }`. 注意, 新的名称必须与原名称不同. 在逻辑上, $rename等价于通过$unset操作符删除掉原有域, 然后再插入一个有newName和原值的文档. 如果目标field不存在, 则不进行任何操作.

<b> 例8: 往students集合中插入一个文档`{_id:20180451, name: '李阳', gender: '男'}`, 然后将name重命名为student_name` </b>

```Javascript
> db.student.insertOne({_id:20180451, name: '李阳', gender: '男'})
{ "acknowledged" : true, "insertedId" : 20180451 }
> db.student.find({_id: 20180451})
{ "_id" : 20180451, "gender" : "男", "student_name" : "李阳" }
```
